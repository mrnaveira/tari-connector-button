...
export function TariWalletSelectionDialog(props: WalletSelectionProps) {
  const { onClose, open, onConnected, signalingServerUrl, snapId, walletConnectProjectId } = props;
  ...
  const onMetamaskClick = async () => {
    const metamaskProvider = new MetamaskTariProvider(snapId, window.ethereum);
  ...
  const onWalletConnectClick = async () => {
    ...
    if (projectId) {
      ...
      console.error("walletConnectProjectId prop not specified");
      handleClose();
    }
  };
